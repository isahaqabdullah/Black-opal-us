...
const DEFAULT_DESCRIPTION =
  'Black Opal Carbons supplies coconut shell activated carbon for United States and North American water treatment, gold recovery, air and gas purification, and industrial applications.';
...
function officeKind(office: Office) {
  ...
  if (text.includes('united states') || text.includes('usa') || text.includes('pittsburgh')) {
    return 'unitedStates';
  }
...
function orderOfficeNetwork(offices: Office[]) {
  const preferredOrder = ['unitedStates', 'india', 'middleEast', 'other', 'headquarters'];
...
const siteId = envValue('SITE_ID', 'black-opal-us');
const regionLabel = envValue('REGION_LABEL', 'United States');
const serviceArea = envValue('SERVICE_AREA', 'United States, Canada, North America');
const marketName = envValue('MARKET_NAME', 'United States and North America');
const utilityMarketLabel = envValue('UTILITY_MARKET_LABEL', regionLabel);
const originStatement = envValue('ORIGIN_STATEMENT', 'Manufactured in India with US-based customer support');
const originDescription = envValue(
  'ORIGIN_DESCRIPTION',
  "coconut shell activated carbon manufactured in India and supplied through Black Opal's US headquarters",
);

const headquarters: Headquarters = {
  name: envValue('HEADQUARTERS_NAME', siteName),
  line1: envValue('ADDRESS_LINE_1', '651 Holiday Dr, STE 400'),
  line2: envValue('ADDRESS_LINE_2', 'Pittsburgh, PA 15220, USA'),
  locality: envValue('ADDRESS_LOCALITY', 'Pittsburgh'),
  region: envValue('ADDRESS_REGION', 'PA'),
  postalCode: envValue('POSTAL_CODE', '15220'),
  country: envValue('ADDRESS_COUNTRY', 'United States'),
  countryCode: envValue('ADDRESS_COUNTRY_CODE', 'US'),
};

const phoneDisplay = envValue('PHONE_DISPLAY', '+1 (412) 928-4970');
const fax = optionalEnvValue('FAX');
const infoEmail = envValue('INFO_EMAIL', 'info@blackopalcarbons.com');
const salesEmail = envValue('SALES_EMAIL', 'info@blackopalcarbons.com');
...
const defaultLogisticsSummary =
  'Company-owned manufacturing, 100 million lbs annual coconut activated carbon capacity, US-based support, and final quality assurance before shipment support consistent supply.';
...
export const siteConfig = {
  ...
  heroLocationProof: envValue(
    'HERO_LOCATION_PROOF',
    'India Manufacturing + US Support',
  ),
