...
const siteOptions = [
  { title: 'US website', value: 'black-opal-us' },
];
...
function siteTitle(siteId?: string) {
  return siteId ? siteTitles[siteId] ?? siteId : 'US';
}
...
export const homePage = {
  ...
  fields: [
    {
      ...
      title: 'Website',
      description:
        'Locks this homepage to one deployed website. This is not a preview switcher; use the matching US Studio to preview that site.',
      type: 'string',
      options: {
        list: siteOptions,
        layout: 'radio',
      ...
      readOnly: (context: { document?: { _id?: string } }) =>
        context.document?._id === 'homePage-black-opal-us',
      validation: (Rule: any) =>
        Rule.custom((value: string | undefined, context: { document?: { _id?: string } }) => {
          ...
          if (context.document?._id === 'homePage-black-opal-us') {
            return value === 'black-opal-us' ? true : 'The US homepage must stay assigned to the US website.';
          }
  ...
  preview: {
    ...
    prepare: ({ siteId }: { siteId?: string }) => ({
      title: `Home Page - ${siteTitle(siteId)}`,
    }),
  },
};
...
export const siteSettings = {
  ...
  preview: {
    select: { siteId: 'siteId' },
    prepare: ({ siteId }: { siteId?: string }) => ({
      title: `Site Settings - ${siteTitle(siteId)}`,
    }),
  },
};
...
export const aboutPage = {
  ...
  preview: {
    select: { siteId: 'siteId' },
    prepare: ({ siteId }: { siteId?: string }) => ({
      title: `About Page - ${siteTitle(siteId)}`,
    }),
  },
};
